...
class laser_gcode(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--stroke-power",                    action="store", type="inkbool",         dest="stroke_power",                        default=True,                           help=" 1.0mm is 100% off power laser .5mm is 50%")
        self.OptionParser.add_option("",   "--passes",                          action="store", type="int",             dest="passes",                              default="1",                            help="Quantity of passes")
        self.OptionParser.add_option("",   "--red-for-passes",                  action="store", type="inkbool",         dest="red_for_passes",                      default=True,                           help="red fill color for number of passes ex red is 1 for 10 passes")
        ...
        self.OptionParser.add_option("",   "--green-for-invisible",             action="store", type="inkbool",         dest="green_is_invisible",                  default=True,                           help="Green stroke #00FF00 is invisible")
...
#                    print_((-f(w[k][i-1]),-f(w[k][i]), [i1[5] for i1 in l1]) )
    ...
    def draw_curve(self, curve, layer, name, group=None, style=styles["biarc_style"]):
        ...
        if group==None:
            ...
            group.set(inkex.addNS('label','inkscape'),"Preview group") #MOD david
        s, arcn = '', 0
        group_sub = inkex.etree.SubElement( group, inkex.addNS('g','svg'))
        group_sub.set(inkex.addNS('label','inkscape'),name)
        ...
        for sk in curve:
            ...
            if s!='':
                if s[1] == 'line':
                    inkex.etree.SubElement( group_sub, inkex.addNS('path','svg'),
                            {
                                'style': style['line'],
...
###
################################################################################
    def generate_gcode(self, curve, layer, depth,stroke_power,passe):
        tool = self.tools
        ...
        #    Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
            ...
            if s[1] == 'move'and (passe == 'start' or passe == 'all'):
                ...
                if lg=="G00"and (not passe == 'no' and not passe=='end'): g += "G1 " + feed + "\n"
                g += "G1 " + c(si[0]) + "\n"
                lg = 'G01'
            elif s[1] == 'arc':
                ...
                if (r[0]**2 + r[1]**2)>.1:
                    ...
                    g += "G1 " + c(si[0]) + " " + feed + "\n"
                    lg = 'G01'
        if si[1] == 'end'and (passe == 'end'or passe == 'all'):
            g += tool['gcode after path'] + "\n"
        return g
...
################################################################################
    def laser(self) :
        ...
        def get_label(objet):
            if objet.attrib.has_key(inkex.addNS('label','inkscape')):
                return objet.get(inkex.addNS('label','inkscape'))
            else:
                 return objet.get('id')

        def is_visible(objet):
            if objet.attrib.has_key('style'): #test si j'ai un element de style car si jamais masque il n'existe pas forcement
                ...
                if style.has_key('stroke'):
                    if int(style['stroke'][3:5],16)>250 and self.options.green_is_invisible:
                        ok_green = False
                    else:
                        ok_green = True
                else:
                    ok_green = True
                if style.has_key('display'): #je verifie que dans le sous dico, il existe l'information, car si jamais masquer, il n'apparait pas
                    if style['display']=='inline': #si inline, afficher
                        ok_visible = True #je renvoie vrai
                    else:
                        ok_visible = False #sinon, masquer je renvoie faux
                else:
                    ok_visible=True
                if ok_visible and ok_green:
                    return True
                else:
                    return False #si jamais afficher, par defaut je renvoie vrai
            else:
                return True #si jamais afficher, par defaut je renvoie vrai

        def is_layer(objet):
            if objet.tag == inkex.addNS('g','svg') and objet.get(inkex.addNS('groupmode','inkscape')) == 'layer': #test de la presencede la clef du la non presence du layer et de la no presence du d
                return True #si jamais afficher, par defaut je renvoie vrai
            else:
                return False #si objamais afficher, par defaut je renvoie vrai

        def is_path(objet):
            if objet.tag == inkex.addNS('path','svg') : #test de la presencede la clef du la non presence du layer et de la no presence du d
                return True #si jamais afficher, par defaut je renvoie vrai
            else:
                return False #si jamais afficher, par defaut je renvoie vrai
        ...
        def get_power(objet):
            if objet.attrib.has_key('style'): #test si j'ai un element de style car si jamais masque il n'existe pas forcement
                ...
                return 1 #si jamais afficher, par defaut je renvoie vrai

        def get_red_stroke(objet): #pour recuperer le nombre de passe
            if objet.attrib.has_key('style'): #test si j'ai un element de style car si jamais masque il n'existe pas forcement
                style= simplestyle.parseStyle(objet.get('style')) # je passe le dico
                if style.has_key('stroke'): #je verifie que dans le sous dico, il existe l'information, car si jamais masquer, il n'apparait pas
                    if style['stroke'] == 'none' or (int(style['stroke'][1:],16)>>16) ==0:
                        return 1
                    else:

                        # if not 0<=style['fill']<=0xffffff: raise ValueError, "valeur inadaptee"
                        return int(style['stroke'][1:],16)>>16
                else:
                    return 1 #sinon, masquer je renvoie faux
        ...
        print_(("paths=",paths))
        for layer in self.layers :
            if layer in paths and is_visible(layer):
                gcode += "\n"+"(Block-name:" + get_label(layer)+ ")"+"\n"
                print_(("layer",layer))
                ...
                for path in paths[layer] :
                    ...
                    if is_visible(path):
                        gcode += "\n"+"(Path:" + get_label(path)+ ")"+"\n"
                            #gestion de la puissance avec la largeur de ligne
                        if self.options.stroke_power and not float(get_power(path))>1:
                            stroke_power = str(int(self.options.laser_power)*float(get_power(path)))
                        else:
                            stroke_power = str(int(self.options.laser_power))
                        #gestion du nombre de passe avec la couleur C de la roue cmjn
                        if self.options.red_for_passes: #test pour savoir si on utilise l'info du niveau de rouge du fil ou le nombre indiquer dans passe. gcode += "G91\nG1 Z-" + self.options.pass_depth + "\nG90\n" + gcode_pass #MOD? voir pour le mettre sur le path directement car on refait tous sinon et le lier a une couleur?
                            red_for_passes= get_red_stroke(path)
                        else:
                            red_for_passes=self.options.passes
                        for i in range(0,red_for_passes):
                            if i==0 : passe= 'start' #juste l'entete
                            if i>0 and i< red_for_passes : passe= 'no' #ni l'un ni l'autre
                            if i== (red_for_passes -1 ): passe= 'end' # juste la find
                            if red_for_passes<=1 : passe = 'all' #on garde les deux
                            p = []
                            dxfpoints = []
                            gcode += "\n"+"(Passe:" + str(i+1)+ " de " + str(red_for_passes) + " / " +str(passe)+')'+"\n"
                            gcode += "G0 Z" + str(round(float(self.options.pass_depth)*i,4)) + "\n" #ajout du Z pour la profondeur
...
################################################################################
    def orientation(self, layer=None) :
        ...
        orientation_group.set(inkex.addNS('label','inkscape'),'Laser axe reference') #MOD David
